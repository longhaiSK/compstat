...
#| fig-width: 10
#| fig-height: 4.2
#| fig-cap: "Illustrates the long-run behaviour of Markov chains by contrasting two random walks on ten states with different transition rules.
#|   Long-run state frequencies for the cyclic walk (left, converges to
...
```{r}
#| label: fig-gibbs-demo
#| fig-width: 10
#| fig-height: 6.5
#| fig-cap: "Demonstrates the Gibbs sampler on a normal model with unknown mean and standard deviation, using trace plots to check convergence and densities to summarize the posterior.
#|   Gibbs sampler on 50 simulated N(10, 2^2) observations: trace plots
...
#| tbl-cap: "Summarizes the posterior distribution obtained from the Gibbs sampler.
#|   Posterior summaries from the Gibbs sampler (after discarding a 200-iteration burn-in)."
...
#| tbl-cap: "Shows that Gibbs sampling extends to rounded (censored) data by treating the exact values as latent variables, and that the parameters are still recovered.
#|   Recovering mu and sigma from data rounded down to the nearest 10, n = 200."
set.seed(3)
...
#| fig-width: 10
#| fig-height: 3.4
#| fig-cap: "Demonstrates random-walk Metropolis-Hastings on a difficult target, a truncated normal distribution, and shows how the step size affects the result.
#|   Random-walk MH targeting N(0, 2^2) truncated to [4, 5], for three step sizes."
...
#| fig-width: 10
#| fig-height: 4.4
#| fig-cap: "Illustrates the trade-off in choosing the step size of a random-walk Metropolis-Hastings sampler, using trace plots and autocorrelation.
#|   Step size too small, about right, and too large: trace (top) and
...
#| fig-width: 10
#| fig-height: 4.4
#| fig-cap: "Illustrates how to tune the step size in practice, by comparing short preliminary runs at several scales.
#|   Preliminary MH runs at four step-size scales, targeting mu (top:
...
#| tbl-cap: "Checks the Metropolis-Hastings sampler against Gibbs sampling on the same problem.
#|   Posterior mean and sd from Metropolis-Hastings versus Gibbs sampling, same data and priors: the two exact samplers of the same posterior agree up to Monte Carlo error."
...
#| fig-width: 10
#| fig-height: 4.4
#| fig-cap: "Compares Hamiltonian Monte Carlo with random-walk Metropolis on a highly correlated target, where the random walk struggles.
#|   Target: bivariate normal with correlation 0.98. Left/middle: first
...
### Shinylive App for Hamiltonian Monte Carlo Tuning {#sec-hmc-app}
...
**Shinylive App Illustrating Hamiltonian Monte Carlo Tuning.** Lets you tune the step size and number of leapfrog steps of Hamiltonian Monte Carlo to see how they affect its exploration of a correlated target. The app below runs the `hmc()` function above on the same correlated target
and lets you vary $\epsilon$ and $L$ directly.
...
#| tbl-cap: "Validates Stan's sampler against the samplers written by hand earlier in the chapter.
#|   Posterior mean and sd: hand-written Gibbs and Metropolis-Hastings versus Stan's NUTS sampler, same data, priors, and model. All three samplers target the identical posterior and agree up to Monte Carlo error."
...
#| fig-width: 10
#| fig-height: 4
#| fig-cap: "Shows a stochastic-volatility model fitted with Stan: the simulated data and the posteriors of the two parameters of interest.
#|   Simulated stochastic-volatility data (left) and posteriors of the
...
```{r}
#| label: tbl-stan-sv
#| tbl-cap: "Reports the convergence diagnostics and posterior summaries of the stochastic-volatility fit.
#|   Posterior summaries for the stochastic-volatility model's top-level parameters, non-centred parameterization: Rhat near 1 and effective sample sizes in the hundreds indicate the chains have converged and mixed, in contrast to the centred version."
