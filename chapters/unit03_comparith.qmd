...
#| fig-width: 9
#| fig-height: 3.6
#| fig-cap: 'Illustrates how floating-point numbers are spaced on the real line in a toy number system: the gaps between representable numbers grow with magnitude, so precision is relative rather than absolute.
#|   Positive numbers at zoom $e=0$ (top, gap $0.1$) and $e=+1$ (bottom, gap $1$). Grey bands hold the crowded marks of all lower zooms ($e<0$, and $e\le 0$ in the bottom panel); overflow occurs only far to the right, beyond $9.9\times10^{9}$.'
...
```{r}
#| label: fig-log1mexp
#| fig-cap: "Compares implementations of log(1 - exp(-u)) to show which formulas remain accurate for very small and very large u.
#|   The three implementations of log(1 - exp(-u)) against u on a log scale. Curves are offset vertically by a small constant so that they remain distinguishable where they agree; log1mexp is the grey reference."
#| fig-width: 10
#| fig-height: 4.2
