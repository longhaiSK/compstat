...
### Significance of the Mantissa: A Zoom Model
...
::: {.pv}
<table>
<tr><th>1</th><th></th><th>exp</th></tr>
<tr><td class="m">d</td><td class="p">×10^</td><td class="e">e</td></tr>
</table>
<p class="cap">One mantissa digit: the ruler marks are $1\times10^{e}, 2\times10^{e}, \ldots, 9\times10^{e}$</p>
:::
...
```{r}
#| label: fig-zoom-model
#| echo: false
#| fig-width: 9
#| fig-height: 3.2
#| fig-cap: "Representable numbers in $[1, 100)$ for a mantissa of 1 and 2 decimal digits. The two zoom levels $[1, 10)$ and $[10, 100)$ are shaded; within each, the marks are evenly spaced, and the spacing is ten times larger at the higher zoom."
par(mar = c(3.5, 5, 1, 1))
plot(NA, xlim = c(1, 100), ylim = c(0.5, 2.5), yaxt = "n", ylab = "", xlab = "", bty = "n")
rect(1, 0.5, 10, 2.5, col = "gray95", border = NA); rect(10, 0.5, 100, 2.5, col = "gray85", border = NA)
axis(1, at = c(1, 10, 20, 40, 60, 80, 100)); axis(2, at = 1:2, labels = c("1 digit", "2 digits"), las = 1, tick = FALSE)
for (t in 0:1) { for (e in 0:1) { m <- 10^e * seq(1, 10 - 10^-t, by = 10^-t)
  segments(m, t + 0.7, m, t + 1.3, lwd = if (t == 0) 2 else 1, col = c("firebrick", "steelblue")[t + 1]) } }
text(c(3.5, 55), 2.45, c("e = 0: gap 1 (1 digit), 0.1 (2 digits)", "e = 1: gap 10 (1 digit), 1 (2 digits)"), cex = 0.8, col = "gray30")
```
...
```{r}
#| label: spacing-demo
2^52 + 1 == 2^52             # spacing is 1: still distinguishable
2^53 + 1 == 2^53             # spacing is 2: the +1 is lost
...
<p class="cap">$10^{10}$: overflow, stored as `Inf`</p>
<table>
<tr><td class="s">+</td><td class="m">1</td><td class="p">.</td><td class="m">0</td><td class="m">0</td><td class="m">0</td><td class="p">×10^</td><td class="x">-10</td></tr>
</table>
<p class="cap">$10^{-10}$: underflow, stored as `0`</p>
...
%| echo: false
%| fig-cap: "Overflow and underflow regions on the number line for double precision"
%| fig-align: center
\usetikzlibrary{decorations.pathreplacing}
\begin{tikzpicture}[scale=1, font=\sffamily]
  \draw[<->, thick] (-6,0) -- (6,0);
  \draw[thick] (0,-0.2) -- (0,0.2) node[above] {$0$};
  \draw[thick] (1.5,-0.2) -- (1.5,0.2) node[above] {$2^{-1074}$};
  \draw[thick] (4,-0.2) -- (4,0.2) node[above] {$2^{1024}$};
  \draw[thick] (-1.5,-0.2) -- (-1.5,0.2) node[above] {$-2^{-1074}$};
  \draw[thick] (-4,-0.2) -- (-4,0.2) node[above] {$-2^{1024}$};
  \draw[decorate,decoration={brace,amplitude=5pt,mirror},blue,thick] (-1.4,-0.5) -- (1.4,-0.5) node[midway,below=5pt] {Underflow ($\to 0$)};
  \draw[decorate,decoration={brace,amplitude=5pt,mirror},red,thick] (4.1,-0.5) -- (5.9,-0.5) node[midway,below=5pt] {Overflow ($+\infty$)};
  \draw[decorate,decoration={brace,amplitude=5pt,mirror},red,thick] (-5.9,-0.5) -- (-4.1,-0.5) node[midway,below=5pt] {Overflow ($-\infty$)};
  \draw[decorate,decoration={brace,amplitude=5pt,mirror},green!50!black,thick] (1.6,-0.5) -- (3.9,-0.5) node[midway,below=5pt] {Representable};
  \draw[decorate,decoration={brace,amplitude=5pt,mirror},green!50!black,thick] (-3.9,-0.5) -- (-1.6,-0.5) node[midway,below=5pt] {Representable};
\end{tikzpicture}
```
...
## smallest double precision floating-point numbers
.Machine$double.xmin   # smallest with full precision, 2^-1022
