...
data.frame(program, quiz.mean.true)
```

The intercept depends on `program`, so we compute it with `ifelse`; the slope is the same for every student:

```{r}
b0.test <- ifelse(program == "Program A", 12, 6)
b1.test <- 0.80

data.frame(program, b0.test, b1.test)
```
...
data.frame(program, b0.final, b1.final)
```

`clip.round` rounds a score and clips it to the valid `0`-`100` range -- we'll reuse it for every test and the final:

```{r}
clip.round <- function(x) round(pmin(100, pmax(0, x)))
```

Now we add random error to the linear predictor to get each student's raw `Test1` score, then clip and round it:

```{r}
error <- rnorm(n, 0, 8)
Test1.raw <- b0.test + b1.test*quiz.mean.true + error

data.frame(program, b0.test, b1.test, quiz.mean.true, error, Test1.raw)
```

```{r}
Test1 <- clip.round(Test1.raw)

data.frame(program, Test1.raw, Test1)
```
...
grep("^Quiz", col_names, value = TRUE)
...
```{r}
# Without the pipe: read from the inside out
round(sqrt(mean(c(4, 9, 16, 25))), 2)

# With the pipe: read top to bottom, left to right
c(4, 9, 16, 25) |>
    mean() |>
    sqrt() |>
    round(2)
...
# Same idea as gen.quiz(), written as one pipeline: each step's output
# fills the *first* argument of the next call, while the remaining,
# named arguments are supplied just as in an ordinary function call
rnorm(20, mean = 75, sd = 10) |>
    round(digits = 0) |>       # round(x, digits = 0)
    pmin(100) |>                # pmin(x, 100)
    pmax(0) |>                  # pmax(x, 0)
    sort(decreasing = TRUE) |>  # sort(x, decreasing = TRUE)
    head(n = 5)                 # head(x, n = 5)
```
...
```{r}
# seq(1, to = 10, by = 2) -- the piped 10 fills the "to" argument
10 |> seq(1, to = _, by = 2)
```
...
### Generating a roster with `mutate()`
...
set.seed(2026)   # a fresh cohort, just for this demonstration

n2 <- 100
students2 <- sprintf("S%03d", 1:n2)
program2  <- factor(sample(c("Program A", "Program B"), n2, replace = TRUE))

data.frame(name = students2, program = program2) |>
    mutate(
        Quiz1 = gen.quiz(75, 20),
        Quiz2 = gen.quiz(78, 20),
        Quiz3 = gen.quiz(72, 20),
        Quiz4 = gen.quiz(80, 20),
        Quiz5 = gen.quiz(74, 20),
        quiz.mean.true = rowMeans(cbind(Quiz1, Quiz2, Quiz3, Quiz4, Quiz5)),
        b0.test  = if_else(program == "Program A", 12, 6),
        b0.final = if_else(program == "Program A", 15, 8),
        Test1 = clip.round(b0.test  + 0.80 * quiz.mean.true + rnorm(n(), 0, 8)),
        Test2 = clip.round(b0.test  + 0.80 * quiz.mean.true + rnorm(n(), 0, 8)),
        Final = clip.round(b0.final + 0.85 * quiz.mean.true + rnorm(n(), 0, 6)),
        across(c(starts_with("Quiz"), starts_with("Test"), Final),
               ~ replace(.x, sample(n(), round(n() * miss.rate)), NA))
    ) -> roster_worktable

roster_worktable |> slice_head (n = 6)

roster_mutate <- roster_worktable |> select(-quiz.mean.true, -b0.test, -b0.final) 

roster_mutate |> slice_head (n= 6)
```
...
### Recomputing course scores with `mutate()` and `rowwise()`
...
```{r}
dplyr_results <- roster |>
    rowwise() |>
    mutate(
        Quiz  = mean(c_across(starts_with("Quiz")), na.rm = TRUE),
        Test  = mean(c_across(starts_with("Test")), na.rm = TRUE),
        Test  = if_else(!is.na(Final) & !is.na(Test) & Final > Test, Final, Test),
        course.score = 0.3 * Quiz + 0.3 * Test + 0.4 * Final
    ) |>
    ungroup() |>
    select(name, program, Quiz, Test, Final, course.score)
...
```{r}
all.equal(as.data.frame(dplyr_results), results, check.attributes = FALSE)
```

### Summarizing by group with `group_by()` and `summarize()`
...
dplyr_results |>
    ...
    summarize(
        n                 = n(),
        mean_quiz         = mean(Quiz, na.rm = TRUE),
        mean_test         = mean(Test, na.rm = TRUE),
        mean_final        = mean(Final, na.rm = TRUE),
        mean_course_score = mean(course.score, na.rm = TRUE),
        .groups = "drop"
    )
```
...
### Filtering and sorting with `filter()` and `arrange()`
...
```{r}
# Failing students, worst first
dplyr_results |>
    filter(course.score < 60) |>
    arrange(course.score) |>
    select(name, program, course.score)

# Top 5 students in the class
dplyr_results |>
    arrange(desc(course.score)) |>
    select(name, program, course.score) |>
    slice_head(n = 5)
```
...
Here we use `tab_spanner()` to group the category scores and the final score under distinct visual headers, and `fmt_number()` to ensure all grades display exactly two decimal places. The column headers below spell out the underlying formulas: the quiz average $\sum_{i}^5{q_i}/5$, the test score $T$, the final exam $F_1$, and the overall course score $F$.

```{r}
#| label: tbl-class-performance-report
#| tbl-cap: "Shows the kind of tidy data frame used to practise data handling in R, with one row per student and one column per assessment category.
#|   Class performance report: first 10 students, with category scores and the final course score."
...
gt(full_results) |>
    ...
    cols_label(
        name = "Name",
        program = "Program",
        Quiz = md("Quizzes ($\\sum_{i}^5{q_i}/5$)"),
        Test = md("Tests ($T$)"),
        Final = md("Final Exam ($F_1$)"),
        course.score = md("Course Score ($F$)")
    ) |>
    # Shrink the name/program columns so the score columns have more room
    cols_width(
        name ~ px(100),
        program ~ px(90)
    ) |>
...
```{r}
#| label: fig-course-score-hist
#| fig-cap: "Demonstrates how to summarize a single numeric variable in R with a histogram, which shows the shape, centre, and spread of the class scores.
#|   Distribution of course scores across the class, with the class average marked in red."
...
```{r}
#| label: fig-course-score-by-program
#| fig-cap: "Demonstrates how to compare a numeric variable across groups in R, using boxplots with jittered points so that both the group summaries and the individual scores are visible.
#|   Course score by academic program, with individual scores jittered over the boxplots."
library(ggplot2)
...
```{r}
#| label: fig-final-vs-quiz-by-program
#| fig-cap: "Demonstrates how to visualize a relationship separately within groups in R, by fitting one regression line per program to a scatterplot.
#|   Final exam score versus mean quiz score, by program, with a fitted regression line per program revealing the built-in intercept offset."
